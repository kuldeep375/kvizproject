...
module.exports.getresponses = async (req, res) => {
    ...
    if (fullname) fullname = JSON.parse(JSON.stringify(fullname));
    let quiz = await db.quiz.findOne({ attributes: ['quiz_id', 'tittle', 'description', 'overall_timer'], where: { quiz_id: req.query.id } });
    if (quiz) quiz = JSON.parse(JSON.stringify(quiz));
    //console.log(quiz);
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        const option = await db.options.findAll({ exclude: ['question_id'], where: { question_id: questions[i].dataValues.question_id } });
        ...
        let responses = await db.responses.findOne({
            where: {
                question_id: questions[i].dataValues.question_id,
                participant_id: req.session.userid,
            },
        });
        if (responses) {
            responses = JSON.parse(JSON.stringify(responses));
            data['response'] = responses.response_statement;
            ...
            for (let j = 0; j < option.length; j++) {
                ...
                if (obj.option_id == responses.option_id) {
