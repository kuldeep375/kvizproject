...
module.exports.addResponse = async (req, res) => {
    try{
        ...
        var query = `INSERT INTO offline_responses ( participant_id, quiz_id, option_id, question_id, response_statement,  is_correct) VALUES (${req.session.userid},${req.body.quiz_id},${JSON.parse(question_resp[i]).response_answer_id},${JSON.parse(question_resp[i]).question_id},'`+stmnt+`',${JSON.parse(question_resp[i]).isCorrect})`;
        // running procedure
